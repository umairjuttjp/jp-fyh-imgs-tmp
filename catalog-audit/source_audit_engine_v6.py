#!/usr/bin/env python3
"""JapansPrime scalable Japanese-source matcher, v6.

V6 is the non-recursive implementation of V5's tightened rules:
- English pack/count attributes are mandatory exactness attributes.
- Multilingual official product-detail pages can pass with exact size/count plus
  meaningful title identity overlap.
- Product-specific brand domains cover KATE and Nihonbashi Kiya.
- Request volume remains bounded by V4's scalable audit flow.
"""
...
import source_audit_engine_v4 as fast_module

# Reload V4 so prior wrapper imports cannot leave monkey-patched functions behind.
fast = importlib.reload(fast_module)
VERSION = "2026-08-02.6"
ORIGINAL_DISCOVER_DOMAINS = fast.discover_domains
...
def improved_discover_domains(product: dict, rows):
    domains, queries = ORIGINAL_DISCOVER_DOMAINS(product, rows)
